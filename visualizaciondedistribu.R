...
ui <- fluidPage(
  titlePanel("Visualización de Distribuciones de Probabilidad Discretas y Continuas GERARDO HERNANDEZ MONDRAGON"),
  sidebarLayout(
    sidebarPanel(
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    ggplot(data.frame(x = dist), aes(x)) +
      geom_histogram(binwidth = 0.5, fill = "red", color = "pink") +
